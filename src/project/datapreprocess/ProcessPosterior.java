...
public class ProcessPosterior {
    ...
    public static void logOddsPosterior(double[][] table, int rareThreshold) {
        ...
        IntStream.range(0, table.length).forEach(i -> {
            if (Arrays.stream(table[i]).sum() <= table[i].length + rareThreshold) {
                ...
                IntStream.range(0, probs.length).forEach(j -> probs[j] = NumericalComputation.logOdds(probs[j]));
            }
        });

        log.info("logOdds");
        log.info("rare {}", rareEvent.get());
        log.info("default {}", defaultProbs);
    }
